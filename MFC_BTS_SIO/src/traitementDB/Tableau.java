		...
		public DefaultTableModel TableFormation(){
			...
			try{
				...
				String query="SELECT F.ID, F.intituleFormation, F.niveauFormation, S.intituleSalle, " +
							 ...
							 "F.ID_formateur=FR.ID AND P.ID=FR.ID_personne ORDER BY F.ID";
		...
		ResultSet rset;
			try{
				...
				String query="SELECT S.ID, P.nom, P.prenom, P.datenaissance, P.sexe, P.adresse, P.ville, P.CP, P.pays, P.mail FROM personne as P, stagiaire as S "+
							  "WHERE P.ID=S.ID_personne ORDER BY S.ID";
				rset=st.executeQuery(query);
...
// Partie Formateur
		...
		ResultSet rset;
			try{
				...
				String query="SELECT F.ID, P.nom, P.prenom, P.datenaissance, P.sexe, P.adresse, P.ville, P.CP, P.pays, P.mail FROM personne as P, formateur as F "+
							  "WHERE P.ID=F.ID_personne ORDER BY F.ID";
				rset=st.executeQuery(query);
		...
		// Tableau Formation 
				public DefaultTableModel TableFormationCat(int IDcat){
					
					DefaultTableModel dt = new DefaultTableModel();
					ResultSet rset;
					try{
						dt.addColumn("ID");
						dt.addColumn("intitulé");
						dt.addColumn("date prochaine session");
						dt.addColumn("durée");
						dt.addColumn("renseignement");
				    	Statement st = new Connect_DB().connexion.createStatement();
						String query="SELECT F.ID, F.intituleFormation, F.niveauFormation, F.dureeFormation " +
									 "FROM formation as F, cat_formation as CF WHERE CF.ID=F.ID_cat_formation AND CF.ID='"+IDcat+"' ORDER BY F.ID";
						rset=st.executeQuery(query);

						while(rset.next()) {
							Object []tableau={rset.getString("ID"),rset.getString("intituleFormation"),rset.getString("niveauFormation"),rset.getString("dureeFormation"), info
							};
							dt.addRow(tableau);
						}
						rset.close();
